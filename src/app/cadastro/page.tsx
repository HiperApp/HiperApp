...
export default function CadastroPage() {
  ...
  const [diagnosticoHipertensao, setDiagnosticoHipertensao] = useState<"sim" | "nao" | null>(null);
  ...
  async function handleCadastro(e: React.FormEvent) {
    ...
    if (diagnosticoHipertensao === null) {
      setErro("Selecione se você já possui diagnóstico de hipertensão arterial.");
      return;
    }
    ...
    const { data, error } = await supabase.auth.signUp({
      email,
      password: senha,
      options: { data: { nome, diagnostico_hipertensao: diagnosticoHipertensao === "sim" } },
    });
          ...
          <div className="flex flex-col gap-2">
            <label
              className={`tap-target flex items-center gap-3 rounded-button border px-4 text-base cursor-pointer ${
                diagnosticoHipertensao === "sim"
                  ? "border-hiper-red bg-hiper-red/5"
            ...
            >
              <input
                ...
                value="sim"
                checked={diagnosticoHipertensao === "sim"}
                onChange={() => setDiagnosticoHipertensao("sim")}
            ...
            >
              <input
                ...
                name="diagnosticoHipertensao"
                value="nao"
                checked={diagnosticoHipertensao === "nao"}
                onChange={() => setDiagnosticoHipertensao("nao")}
                className="w-5 h-5 accent-hiper-red"
              />
