...
import { tendencia, nivelSemana, textoResumoSemana } from "@/lib/tendencia";
import { dicaDoPeriodo } from "@/lib/dicaDoDia";
import { DICAS_IMPORTANTES } from "@/lib/classification";
...
export default async function DashboardPage() {
  ...
  // Resumo semanal: mesma lógica usada na tela de Evolução, olhando
  // apenas os últimos 7 dias, para o usuário não precisar trocar de aba.
  const seteDiasAtras = new Date();
  seteDiasAtras.setDate(seteDiasAtras.getDate() - 7);
  const medicoesSemana = (medicoes ?? [])
    .filter((m) => new Date(m.data + "T00:00:00") >= seteDiasAtras)
    .sort((a, b) => (a.data + a.horario).localeCompare(b.data + b.horario));
  const sistolicasSemana = medicoesSemana.map((m) => m.pressao_sistolica);
  const diastolicasSemana = medicoesSemana.map((m) => m.pressao_diastolica);
  const tendenciaSemana = tendencia(sistolicasSemana);
  const resumoSemana =
    medicoesSemana.length >= 2
      ? textoResumoSemana(tendenciaSemana, nivelSemana(sistolicasSemana, diastolicasSemana))
      : null;
  ...
  return (
    <div className="px-6 pt-14 pb-6">
      ...
      <Card className="mb-6">
        <p className="text-gray-500 text-sm mb-2">Última aferição</p>
        {ultima ? (
          <>
      ...
      {resumoSemana && (
        <Card className="mb-6 flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-gray-700">Resumo da semana</p>
            <p className="text-sm text-gray-500">{resumoSemana.texto}</p>
          </div>
        </Card>
      )}
