...
export default function NovaMedicaoPage() {
  ...
  async function handleSalvar(e: React.FormEvent) {
    ...
    // Antes o código usava "user!.id", que quebra silenciosamente (exceção
    // não tratada) se a sessão ainda não tiver sido restaurada — o botão
    // ficava travado em "Salvando..." para sempre, sem nenhuma mensagem de
    // erro. Agora tratamos esse caso explicitamente.
    if (!user) {
      setSalvando(false);
    ...
    if (error) {
      ...
      setErro(
        `Não foi possível salvar. ${error.message ? `Detalhe: ${error.message}` : "Tente novamente."}`
      );
      return;
    }
